...
Total_cycles_NAMD_GOMC_sims = 2 # enter integer: # simulations NAMD + GOMC sims = 2 * Total_cycles_NAMD_GOMC_sims
...
No_core_box_0 =  16  # CPU cores: this is the ONLY place to enter CPU cores for 'GCMC', 'NPT', 'NVT', and  'GEMC' and only_use_box_0_for_NAMD_for_GEMC = True
No_core_box_1 =  0  # CPU cores:  is always ZERO for 'GCMC', 'NPT', 'NVT' , and  'GEMC' and only_use_box_0_for_NAMD_for_GEMC = True

#Physical/Chemical properties Properties
simulation_Temp_K = 500 # float: GOMC and NAMD units of temperature are in K
simulation_Pressure_bar =  1.01325 # 1.01325 bar = 1 atm # float: GOMC and NAMD units of pressure are in bar
# Currently, a total of only 4 chemicals/residues can be used for GCMC simulations
GCMC_only_ChemPot_Type_1_Residue = 'H2O'  # string: GCMC ONLY!!! -- GOMC residue name of the molecule, must have Type_1 for GCMC
GCMC_only_ChemPot_Type_1_mu_K = -5000  # float: GCMC ONLY!!! -- GOMC units of mu (chemical potential ) are in units of K, must have Type_1 for GCMC
...
GOMC_Run_Steps = 1000  # needs to be 10 minimum for now, NEEDS TO BE THE SAME AS THE PREVIOUS SIMULATION, IF RESTARTED!
NAMD_Run_Steps = 100   # needs to be 10 minimum for now, NEEDS TO BE THE SAME AS THE PREVIOUS SIMULATION, IF RESTARTED!
NAMD_Minimize_mult_scalar = 10    # interger, NAMD_minimize steps = NAMD_Run_Steps * NAMD_Minimize_mult_scalar
...
Starting_FF_file = '/required_data/equilb_box_500K/pore_3x3x2.8nm_1-layer.inp'  # from main folder

# box 0 data
Starting_PDB_box_0_file = '/required_data/equilb_box_500K/pore_3x3x2.8nm_1-layer.pdb' # from main folder
Starting_PSF_box_0_file = '/required_data/equilb_box_500K/pore_3x3x2.8nm_1-layer.psf' # from main folder

# box 1 data
Starting_PDB_box_1_file = 'required_data/build_liq_vap_boxes/box_1_vap_water_200A_L_cubed_box_350K.pdb' # from main folder
Starting_PSF_box_1_file = 'required_data/build_liq_vap_boxes/box_1_vap_water_200A_L_cubed_box_350K.psf' # from main folder
...
GOMC_Hist_sample_Steps  = int( GOMC_Run_Steps )
...
def write_NAMD_conf_file(Python_file_directory, path_NAMD_template, path_NAMD_runs, GOMC_newdir,
    ...
    if Run_No != 0:
        ...
        new_NAMD_data = NAMD_Template_data.replace("parameter_file",
                                                   str(Python_file_directory) + "/" + Starting_FF_file)
...
def write_GOMC_conf_file(Python_file_directory, path_GOMC_runs, Run_No, GOMC_Run_Steps,
    ...
    run_GOMC_copy_CkPt_new_dir_command = "cp " + str(previous_GOMC_dir) + "/" + "checkpoint.dat" + " " \
                                         + str(GOMC_newdir)
...
for Run_No in range(Starting_sims_NAMD_GOMC, Total_sims_NAMD_GOMC):
    ...
    if Run_No % 2 == 0:  # NAMD's run time. :# NAMD's staring run.  NAMD starts simulation series with energy minimization
        ...
        if Run_No != 0:
            ...
            # for box_0
            cp_NAMD_box_0_FFT_Run_0_new_dir_cmd = "cp " + str(NAMD_box_0_Run_0_dir) + "/" \
