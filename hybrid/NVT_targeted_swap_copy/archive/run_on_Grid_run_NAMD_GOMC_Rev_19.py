...
Total_cycles_NAMD_GOMC_sims = 10000 # 400000 # enter integer: # simulations NAMD + GOMC sims = 2 * Total_cycles_NAMD_GOMC_sims
Starting_at_cycle_NAMD_GOMC_sims = 0    # enter integer: enter 0 for intial simualtion start, other for restart

#  GPU/CPU selection, Simulation type (ensemble), cores in each box,
Use_CPU_or_GPU =  'CPU'   # enter string : 'CPU' or 'GPU', currently only CPU available,  you need to provide the NAMD exectutable GPU code as well
simulation_type = 'GCMC'  # enter string : 'GEMC', 'GCMC', 'NPT', 'NVT' (only GEMC-NVT available currently: 'GEMC' = GEMC-NVT)
...
No_core_box_0 =  8  # CPU cores: this is the ONLY place to enter CPU cores for 'GCMC', 'NPT', 'NVT', and  'GEMC' and only_use_box_0_for_NAMD_for_GEMC = True
...
simulation_Temp_K = 298 # float: GOMC and NAMD units of temperature are in K
simulation_Pressure_bar =  1.01325 # 1.01325 bar = 1 atm # float: GOMC and NAMD units of pressure are in bar (must have number even if unused)
# Currently, a total of only 4 chemicals/residues can be used for GCMC simulations
GCMC_only_ChemPot_Type_1_Residue = 'WAT'  # string: GCMC ONLY!!! -- GOMC residue name of the molecule, must have Type_1 for GCMC
GCMC_only_ChemPot_Type_1_mu_K = -3800  # float: GCMC ONLY!!! -- GOMC units of mu (chemical potential ) are in units of K, must have Type_1 for GCMC
...
GOMC_Run_Steps = 4000  # needs to be 10 minimum for now, NEEDS TO BE THE SAME AS THE PREVIOUS SIMULATION, IF RESTARTED!
NAMD_Run_Steps = 2000   # needs to be 10 minimum for now, NEEDS TO BE THE SAME AS THE PREVIOUS SIMULATION, IF RESTARTED!
...
Starting_FF_file_GOMC = 'required_data/equilb_box_298K/water_box_rigid_FF.inp'  # from main folder
Starting_FF_file_NAMD = 'required_data/equilb_box_298K/NAMD_24ns_Equilb_NVT_water.inp'  # from main folder

# box 0 data
Starting_PDB_box_0_file = 'required_data/equilb_box_298K/NAMD_24ns_Equilb_NVT_water.pdb' # from main folder
Starting_PSF_box_0_file = 'required_data/equilb_box_298K/NAMD_24ns_Equilb_NVT_water.psf' # from main folder

# box 1 data
Starting_PDB_box_1_file = 'required_data/equilb_box_298K/water_reservoir_box.pdb' # from main folder
Starting_PSF_box_1_file = 'required_data/equilb_box_298K/water_reservoir_box.psf' # from main folder

# MD/MC engine file names and paths, if the paths are required
NAMD_executable_file = '/wsu/home/hf/hf68/hf6839/NAMD_2.14_Linux-x86_64-multicore/namd2'
GOMC_executable_file ='/wsu/home/hf/hf68/hf6839/GOMC-development_1_11_21/bin/GOMC_'+ str(Use_CPU_or_GPU) + '_'  + str(simulation_type)

#********************************************
# run on my Desktop with below (start)
#********************************************
# MD/MC engine file names and paths, if the paths are required
...
#********************************************
# run on my Desktop with below (end)
#********************************************
...
use_GOMC_checkpoint = 'false' # select 'true' or 'false', needs to be false for now as does not print seperate PSF's
...
#*************************************************
# create NAMD and GOMC folders (start)
#*************************************************
path_NAMD_runs = "NAMD"
...
#*************************************************
if simulation_Pressure_bar == None and simulation_type in ['NPT', 'GEMC-NPT'] \
        and (isinstance(simulation_Pressure_bar, float) == False or isinstance(simulation_Pressure_bar, int) == False):
    if simulation_Pressure_bar < 0:
        raise Exception("The simulation pressure needs to be set for the NPT, GEMC-NPT simulation types. \n")
elif simulation_Pressure_bar == None and simulation_type in ['NVT', 'GEMC-NVT', 'GCMC']:
