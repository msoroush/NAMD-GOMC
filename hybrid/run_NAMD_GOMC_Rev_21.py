...
profilerStart("Start")
print("\nCalling a range push to mark this init of python: ")
profilerRangePush("PYINIT")
...
Total_cycles_NAMD_GOMC_sims = 2 # 400000 # enter integer: # simulations NAMD + GOMC sims = 2 * Total_cycles_NAMD_GOMC_sims
...
GCMC_only_ChemPot_Type_1_mu_K = -4500  # float: GCMC ONLY!!! -- GOMC units of mu (chemical potential ) are in units of K, must have Type_1 for GCMC
...
Starting_FF_file_GOMC = 'required_data/equilb_box_500K/common/SPCE.par'  # from main folder
Starting_FF_file_NAMD = 'required_data/equilb_box_500K/common/toppar_water_ions.str'
# box 0 data

Starting_PDB_box_0_file = 'required_data/equilb_box_500K/common/water_sys.pdb'
Starting_PSF_box_0_file = 'required_data/equilb_box_500K/common/water_sys.psf'
# box 1 data
Starting_PDB_box_1_file = 'required_data/equilb_box_500K/common/water_reservoir.pdb'
Starting_PSF_box_1_file = 'required_data/equilb_box_500K/common/water_reservoir.psf'

# MD/MC engine file names and paths, if the paths are required
NAMD_executable_file = str(os.getcwd()) + '/required_data/bin/NAMD212/namd3'
#NAMD_executable_file = str(os.getcwd()) + '/required_data/bin/NAMD212/namd3_' + str(Use_CPU_or_GPU)
...
def write_GOMC_conf_file(Python_file_directory, path_GOMC_runs, Run_No, GOMC_Run_Steps,
    ...
    new_GOMC_data = new_GOMC_data.replace("coor_box_0_file",
                                          NAMD_box_0_newdir + '/namdOut.restart.coor')
    new_GOMC_data = new_GOMC_data.replace("xsc_box_0_file",
                                          NAMD_box_0_newdir + '/namdOut.restart.xsc')
    ...
    # Read the angles from the intial/starting PDB file
    NAMD_xsc_box_0_file = NAMD_box_0_newdir + "/namdOut.restart.xsc"
    ...
    run_GOMC_copy_CkPt_new_dir_command = "ln -sf " + str(previous_GOMC_dir) + "/" + "checkpoint.dat" + " " \
                                         + str(GOMC_newdir)
...
for Run_No in range(Starting_sims_NAMD_GOMC, Total_sims_NAMD_GOMC):
    ...
    if Run_No % 2 == 0:  # NAMD's run time. :# NAMD's staring run.  NAMD starts simulation series with energy minimization
        ...
        if Run_No != 0:
            ...
            # for box_0
            cp_NAMD_box_0_FFT_Run_0_new_dir_cmd = "ln -sf " + str(NAMD_box_0_Run_0_dir) + "/" \
        ...
        if Run_No == 0:
            profilerRangePop("PYINIT")
        else:
            profilerRangePop("PYGOMC2NAMD")
